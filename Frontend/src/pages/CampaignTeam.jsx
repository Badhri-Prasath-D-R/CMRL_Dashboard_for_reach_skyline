...
const CampaignTeam = () => {
  ...
  const fetchTasks = async () => {
    try {
      const response = await api.get('/tasks?team=campaign');
  ...
  const fetchEmployees = async () => {
    try {
      const response = await api.get('/employees?team=campaign');
  ...
  return (
    <div className="container">
      <h1 style={{ textAlign: 'center', color: '#007bff' }}>Campaign Team Dashboard</h1>
...
const TaskRow = ({ task, employees, onAssign, onDelete, onUpdate }) => {
  ...
  return (
    <tr>
      <td>{task.client}</td><td>{task.clientID}</td><td>{task.deliveryDate}</td><td>{task.activityCode}</td><td>Campaign (AT011)</td>
