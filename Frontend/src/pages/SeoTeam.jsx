...
const SeoTeam = () => {
  ...
  const fetchTasks = async () => {
    try {
      const response = await api.get('/tasks?team=seo');
  ...
  const fetchEmployees = async () => {
    try {
      const response = await api.get('/employees?team=seo');
  ...
  return (
    <div className="container">
      <h1 style={{ textAlign: 'center', color: '#007bff' }}>SEO Team Dashboard</h1>
      <div className="table-container">
        <table>
          <thead>
            <tr>
              <th>Client</th><th>ID</th><th>Date</th><th>Code</th><th>Type</th><th>Description</th> 
              <th>Min</th><th>Amt</th><th>Link</th><th>Status</th><th>Remarks</th><th>Action</th><th>Assign</th>
            </tr>
          </thead>
          <tbody>
            {tasks.length === 0 ? (
...
const TaskRow = ({ task, employees, onAssign, onDelete, onUpdate }) => {
  ...
  return (
    <tr>
      <td>{task.client}</td><td>{task.clientID}</td><td>{task.deliveryDate}</td><td>{task.activityCode}</td><td>SEO (AT010)</td> 
